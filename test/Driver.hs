...
tests = do
  ...
  t11 <- TD.tasty (TD.description "symlinksNotFollowed" <> TD.name "ConfigTest.tasty_symlinksNotFollowed") ConfigTest.tasty_symlinksNotFollowed

  t12 <- testCase "listCompare" DiscoverTest.unit_listCompare

  t13 <- pure $ QC.testProperty "additionCommutative" DiscoverTest.prop_additionCommutative

  t14 <- pure $ SC.testProperty "sortReverse" DiscoverTest.scprop_sortReverse

  t15 <- HS.testSpec "prelude" DiscoverTest.spec_prelude

  t16 <- testGroup "addition" DiscoverTest.test_addition

  t17 <- testGroup "multiplication" DiscoverTest.test_multiplication

  t18 <- testGroup "generateTree" DiscoverTest.test_generateTree

  t19 <- testGroup "generateTrees" DiscoverTest.test_generateTrees

  t20 <- TD.tasty (TD.description "reverse" <> TD.name "DiscoverTest.tasty_reverse") DiscoverTest.tasty_reverse

  t21 <- TD.tasty (TD.description "skip me" <> TD.name "DiscoverTest.tasty_skip_me") DiscoverTest.tasty_skip_me

  t22 <- TD.tasty (TD.description "tree of testCase not skipped" <> TD.name "DiscoverTest.tasty_tree_of_testCase_not_skipped") DiscoverTest.tasty_tree_of_testCase_not_skipped
